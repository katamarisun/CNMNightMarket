...
void main() 
{
    ...
    gl_Position = hpos;
    if (use_height) {
        float height = texture2D(heightSampler, fUV)[0];
        height = height * 2.0 - 1.0;
        vec3 norm_for_height = Normal;
        if (use_normal) {
            norm_for_height = texture2D(normalSampler, fUV).xyz;
        }
        gl_Position = hpos + fudgeHeightDisp * height * vec4(norm_for_height, 1.0);
    }
